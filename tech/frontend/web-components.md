...
MDN より引用。要はコンポーネントごとに分割して再利用することができるコードにするってことか。React や Vue などのコンポーネントベースのフレームワークが最近人気ですが、フレームワークを使わなくても Web の標準に乗っかっていい感じに効率の良いコードを書くことができるのはいいことですね。
...
また、38行目と68行目でそれぞれ `style.isConnected` を出力しています。出力結果は38行目が `false`、68行目が `true` になっています。  
これは `style` がアタッチされるまで適用されないことを表しています。
